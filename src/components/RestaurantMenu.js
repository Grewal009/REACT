...
const RestaurantMenu = () => {
  ...
  async function getMenu() {
    const data = await fetch(
      "https://www.swiggy.com/dapi/menu/pl?page-type=REGULAR_MENU&complete-menu=true&lat=30.7016176&lng=76.820049&restaurantId=" +
        id
    );
    ...
    <div className="flex justify-center">
      <div className="w-[374px] sm:w-[550px] md:w-[680px] lg:w-[850px]">
        <div className="text-center">
          ...
          <h2 className="text-4xl font-bold leading-8 mb-1">
            {restaurant?.cards[0]?.card?.card?.info?.name}
          </h2>

          <h3 className="text-lg font-medium leading-5 mb-1">
          ...
          <h5 className="text-sm leading-4 mb-1">
            Total Rating {restaurant?.cards[0]?.card?.card?.info?.totalRatings}+
            ratings
          </h5>
          ...
          {restaurant?.cards[2]?.groupedCard?.cardGroupMap?.REGULAR?.cards[1]?.card?.card?.itemCards?.map(
            (rest) => (
              <li
                ...
                className="p-1  mx-5 my-5 flex flex-col justify-between shadow-md shadow-slate-900"
              >
                <div className="flex justify-between items-center">
                  ...
                  <div>
                    ...
                    <button
                      onClick={() => handleAddItem(rest.card.info)}
                      className="w-32 h-9 text-slate-600 bg-green-300 rounded-md hover:font-bold"
                    >
                      Add to cart
                    </button>
                  </div>
                ...
                <h1 className="text-lg font-bold leading-4 mb-1">
                  {rest?.card?.info?.name}
                </h1>

                <h3 className="text-xs font-medium leading-3">
